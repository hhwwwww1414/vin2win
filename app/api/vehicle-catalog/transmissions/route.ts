...
import { getVehicleCatalogTransmissions } from '@/lib/server/vehicle-catalog';
...
export async function GET(request: Request) {
  try {
    ...
    const items = await getVehicleCatalogTransmissions({
      generationId,
      fuelTypeId,
      driveTypeId,
      bodyTypeId,
    ...
    const message = error instanceof Error ? error.message : 'Не удалось загрузить коробки.';
