...
import { bulkUpdateWantedListings } from '@/lib/server/moderation';
...
export async function POST(request: Request) {
  try {
    ...
    const result = await bulkUpdateWantedListings({
    ...
    const message = error instanceof Error ? error.message : 'Не удалось выполнить массовое действие для запросов на подбор.';
